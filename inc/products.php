...
function get_list_view_html($product){
	...
	$output .= '<img src="' . BASE_URL . $product["img"] . '" alt="' . $product["name"] . '" title="' . "desc for " . $product["name"] . '" />';
...
function get_products_search($searchTerm){
	$results = array();
	$all = get_products_all();
	foreach ($all as $product_id => $product) {
//		if(is_string($searchTerm) && stripos($product["name"], $searchTerm)){
//			$results[] = $product;
//		}else if(is_int(intval($searchTerm)) && $product_id == $searchTerm){
//			$results[] = $product;
//		}
